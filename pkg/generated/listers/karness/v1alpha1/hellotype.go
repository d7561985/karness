...
type HelloTypeLister interface {
	...
	List(selector labels.Selector) (ret []*v1alpha1.HelloType, err error)
...
// List lists all HelloTypes in the indexer.
func (s *helloTypeLister) List(selector labels.Selector) (ret []*v1alpha1.HelloType, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha1.HelloType))
	})
	return ret, err
}
...
type HelloTypeNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1alpha1.HelloType, error)
...
// List lists all HelloTypes in the indexer for a given namespace.
func (s helloTypeNamespaceLister) List(selector labels.Selector) (ret []*v1alpha1.HelloType, err error) {
...
// Get retrieves the HelloType from the indexer for a given namespace and name.
func (s helloTypeNamespaceLister) Get(name string) (*v1alpha1.HelloType, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1alpha1.Resource("hellotype"), name)
	}
	return obj.(*v1alpha1.HelloType), nil
}
